...
myparser.add_argument("--testcutoff", action = "store", dest = "hitcutoff", default = 0, type = int, help = "experimental: include markers absent in this many comparison genomes")
...
def read_PO_file(filename):
	#cutoff_perc = args.hitcutoff #testing request that markers present in MOST but not ALL are included
	max_nohits = args.hitcutoff
	...
	for line in open_PO_file:
		...
		if firstline:
			...
			firstline = False
			#max_nohits = len(headers) - ((len(headers) / 100) * cutoff_perc)
			mylogger.info(" cutoff is {} --> not more than {} missing instances of each marker allowed".format(max_nohits, max_nohits))
		elif not line.startswith("#"):
			if not "," in line: #ignore line if multiple orthologs for ANY comparison organism
				if line.count("*") <= max_nohits: #ignore line if ortholog is missing for MORE than "max_nohits" comparison organism
					...
					for zt in range(len(zeilentokens)):
						...
						if zeilentokens[zt] == "*":
							zeilentokens[zt] = "{}_{}_MISSING_MARKER".format(headers[zt], lcounter) #not sure why i shortened it to first five chars earlier. However, on case some comparison orrgs are not unique in first 5 chars, i betetr use the whole name
							#zeilentokens[zt] = "{}_{}_MISSING_MARKER".format(headers[zt][:5], lcounter)
...
def read_fasta_seqs(headers, MLSA_list):
	...
	for h in range(len(headers)):
		...
		for temprecord in temprecord_dict:
			if temprecord.endswith("_MISSING_MARKER"):
				record_dict[headers[h]][temprecord] = SeqRecord(Seq("XXXXXXXXXXXXXXXXXXXXXXX"), id = temprecord, name = temprecord)
				mylogger.debug(" !! {} : length = {}".format(record_dict[headers[h]][temprecord].id, len(record_dict[headers[h]][temprecord])))
...
def write_temp_files(record_dict, MLSA_list, prefix):
	...
	for locusindex in range(len(MLSA_list[0][1])):
		...
		templist = []
		for orgindex in range(len(MLSA_list)):
			templist.append(record_dict[MLSA_list[orgindex][0]][MLSA_list[orgindex][1][locusindex]])
			#SeqIO.write([record_dict[MLSA_list[orgindex][0]][MLSA_list[orgindex][1][locusindex]]], open_tempfile, "fasta")
			mylogger.debug("   {} --> length = {}".format(templist[-1].id, len(templist[-1]))) 
			if templist[-1].id.endswith("MISSING_MARKER"):
				mylogger.debug("------")
				mylogger.debug(templist[-1])
				mylogger.debug("-----")
		maxlen = max([ len(x) for x in templist ])
		mylogger.debug(" MAXLEN : {}".format(maxlen))
		for rindex in range(len(templist)):
			if templist[rindex].id.endswith("MISSING_MARKER"):
				templist[rindex].seq = Seq("X" * maxlen)
				#print templist[rindex].id
				#print templist[rindex].seq
		SeqIO.write(templist, open_tempfile, "fasta")
		open_tempfile.close()
		mylogger.debug("\n###################################\n")
...
def rename_after_gblocks(align_file, temp_name_dict, final_filename):
	...
	for al in alignment:
		#change "missing marker" seqeunces from ">" to "-"
		#print "renaming " + al.id + " to " + temp_name_dict[al.id]
		if al.seq.count("-") + al.seq.count("X") == len(al.seq):
			al.seq = Seq("-" * len(al.seq))
